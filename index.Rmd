...
# this is markdown to import the image at the begginning of the web page
...
## bring data (at this stage data is extracted per acoustic points in arcPro, but working with Nick Murray to have it automated in GEE)
...
## getting rid of the -9999.000 and 0 values in variables and rows
EOMAP_clean <- filter(RegressionDataset, Depth != 0, SDB_10m != -9999.00, SDB_15m != -9999.00)
## change sign in acoustic depth readings
...
## getting variables for regression (0-5 m)

split0_3 <- filter(EOMAP_to_regress, Depth > -3, SDB_15m > -3, SDB_10m > -3 )  
Acoustic_depth_0_3m<-split0_3$Depth
EOMAP_Landsat_Depth_0_3m <- split0_3$SDB_15m
EOMAP_Sentinel_Depth_0_3m <- split0_3$SDB_10m
...
## getting variables for regression (0-10 m)
...
## getting variables for regression (0-15 m)
...
## linear model fitting (0-3) (I added this later that's why I named them not in sequence)

fit0_3_a <-lm(EOMAP_Landsat_Depth_0_3m ~ Acoustic_depth_0_3m)
fit0_3_b<-lm(EOMAP_Sentinel_Depth_0_3m ~ Acoustic_depth_0_3m)


## linear modelling fitting (0-3)
...
## these were tests to produce the plots individually but then realize the function made a better job (see function below)
...
## function to get the regression plot and annotation of regression parameters over the plot

ggplotRegression <- function (fit) { 
require(ggplot2)
ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
  geom_point() + theme(panel.background = element_rect(fill = "#BFD5E3", colour = "#6D9EC1",
...
- **EOMAP Landsat depth vs acoustic data (0-3 meter subset)**


```{r, echo = FALSE}
ggplotRegression(fit0_3_a)

```

- **EOMAP Sentinel depth vs acoustic data (0-3 meter subset)**

```{r, echo = FALSE}
ggplotRegression(fit0_3_b)
